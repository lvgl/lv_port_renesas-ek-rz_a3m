...
#endif
/* RZ/A3M LCD */
/* 0x00 */ 0x48, 0x00, 0x05, 0x20, 0x03, 0x05, 0xbc, 0x10, 0x01, 0x08, 0x1e, 0x05, 0x46,
    0x32, 0x03, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x18, 0x1a, 0x1e,
    0x14, 0x8b, 0x2b, 0x0e, 0x20, 0x22, 0x33, 0x0f, 0x00, 0x00, 0x00, 0xc2, 0x33,
    0x1c, 0x00, 0x00, 0x55, 0x0a, 0x00, 0x2b, 0xe0, 0x03, 0x32, 0x46, 0x32, 0x1c,
    0x30, 0x02, 0xc2, 0x0a, 0x05, 0x05, 0x00, 0x04, 0x96, 0x1d, 0x00, 0x8d, 0x20,
    0x00, 0x83, 0x24, 0x00, 0x7c, 0x28, 0x00, 0x75, 0x2d, 0x00, 0x75, 0x00, 0x00,
    ...
    0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x24, 0x26, 0xff, 0xff, 0xff, 0xff, 0xff,
...
void reset_gt911(void)
{
        ...
        R_IOPORT_PinCfg(&g_ioport_ctrl, DISP_INT,  ((uint32_t) IOPORT_PERIPHERAL_MODE1 | (uint32_t) IOPORT_CFG_PORT_DIRECTION_INPUT));
#endif
}
...
fsp_err_t init_ts(i2c_master_ctrl_t * p_api_ctrl)
{
    ...
    err = R_RIIC_MASTER_SlaveAddressSet( p_api_ctrl, GT_911_I2C_ADDRESS, I2C_MASTER_ADDR_MODE_7BIT);
    if (FSP_SUCCESS == err)
    {
...
fsp_err_t enable_ts(i2c_master_ctrl_t * p_api_i2c_ctrl, external_irq_ctrl_t * p_api_irq_ctrl)
{
    ...
    err = R_RIIC_MASTER_SlaveAddressSet( p_api_i2c_ctrl, GT_911_I2C_ADDRESS, I2C_MASTER_ADDR_MODE_7BIT);
    if (FSP_SUCCESS == err)
    {
...
//             printf("0x%x, ", g_read_config[x]);
             printf("REG 0x%x = 0x%02x\n\r", GT911_REG_CONFIG_VERSION + x, g_read_config[x]);
         }
    }
#endif
    if (FSP_SUCCESS == err)
    {
        ...
        if (FSP_SUCCESS == err)
        {
            ...
            {
                err = R_INTC_IRQ_ExternalIrqEnable(p_api_irq_ctrl);
            }
        }
    }
